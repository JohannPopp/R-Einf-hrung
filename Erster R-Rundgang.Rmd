...
`r newthought("Wenn man die gleiche Berechnung")` oft wiederholen will, lohnt es sich vielleicht, mit `function()` einen eigenen Befehl dafür zu programmieren.
```{r}
...
```
```{marginfigure}
`function()` definiert neue Befehle
```
  
Probieren Sie ihren neuen Befehl mit verschiedenen Daten aus.
```{r}
Mittelwert(alter)
Mittelwert(c(18, 36, 22, 54, 30, 28, 29, 19))
```
...
`r newthought("Sie haben jetzt schon einige Befehle")``r margin_note("Struktur von Befehlen in R")` von R kennen gelernt: `licence()`, `c()`, `sum()`, `length()`, `function()` und `mean()`. All diese Befehle bestehen aus einem Namen, gefolgt von weiteren Angaben in Klammern.  

Befehle werden in R _Funktionen_ genannt und die Angaben, die in den Klammern folgen, heißen _Argumente_. Die allermeisten Funktionen beinhalten als erstes Argument die Daten, auf die die Funktion angewendet werden soll. Weitere Argument können (durch Kommata getrennt) folgen. Jede vorgefertigte Funktion hat eine Hilfeseite, auf der genau erklärt wird, wie sie funktioniert.
...
```
Wenn Sie damit nicht weiter kommen, können Sie gesuchter Inhalte `r margin_note("Inhalte mit ?? suchen")`mit zwei vorangestellten Fragezeichen eingeben. Es werden dann alle geladenen Hilfe-Seiten nach diesem Begriff durchsucht. Wenn Sie z.B. Hilfe zum Kolmogorov-Smirnov-Test suchen, geben Sie `??Kolmogor` ein. Es wird Ihnen dann angezeigt, dass es eine Funktion `ks.test()` gibt, die diesen Test berechnet.
...
`r newthought("Normalerweise werden statistische Datensätze")` in Form `r margin_note("data.frame()")` einer Datenmatrix angelegt, bei der jede Zeile einer statistischen Einheit (z.B. einer befragten Person) entspricht und jede Spalte einer Variable (z.B. Frage). In R ist so eine Datenmatrix ein `data.frame()`.  
...
installieren. Der Befehl läd das Paket automatisch herunter und installiert es. (Eventuell wird beim ersten Mal abgefragt, von welchem *Miror* Sie es herunter laden wollen.)  
...
```{r fig.asp = 0.8, fig.cap="Streudiagramm-Matrix des Datensatzes 'dat' (fiktive Daten)."}
plot(dat)
```

Ein paar Beispiele können Sie sich mit folgender Eingabe ansehen:
```{r, eval=FALSE}
demo(graphics)
```
